...
public class CharacterAnalyser {
	...
	public static ArrayList<Rect> getBoundingRects(Mat image) {
		...
		Imgproc.morphologyEx(localImage, localImage, Imgproc.MORPH_CLOSE, Imgproc.getStructuringElement(Imgproc.MORPH_RECT, new Size(17, 3)));
	...
	public static ArrayList<ArrayList<Point[]>> isolateGlyphs(Mat image, ArrayList<Rect> boundingRect) {
		...
		localImage = new GaussianAdaptiveThreshold(new GrayScale()).execute(localImage);
		...
		for (int i = 0; i < boundingRect.size(); i++) {
			...
			for (int j = 1; j < w - 1; j++) {
//				System.out.println(breakCosts[j - 1] + " " + breakCosts[j] + " " + breakCosts[j + 1]);
	...
	public static Mat analyse(Mat image, ArrayList<ArrayList<Point[]>> glyphs) {
		...
		for (ArrayList<Point[]> word : glyphs) {
			for (int i = 0; i < word.size(); i++) {
				Imgproc.rectangle(localImage, word.get(i)[0], word.get(i)[1], new Scalar(AlgorithmDecorator.COLOR_GRAY), 1);
			}
		}
		...
		// for (int j = 0; j < zeroCols.size() - 1; j++) {
		//
		// Imgproc.rectangle(localImage, new Point(zeroCols.get(i), y), new
		// Point(zeroCols.get(i + 1), y + h), color, 1);
		//
		//// Graph<Point> symbol = new Graph<Point>();
		//// HashMap<Integer, Integer> invariants = new HashMap<Integer,
		// Integer>();
		//// HashMap<Point, ArrayList<Point>> blackRowsList = new HashMap<Point,
		// ArrayList<Point>>();
		//// LinkedHashSet<ArrayList<Point>> blackRowsOrderedList = new
		// LinkedHashSet<ArrayList<Point>>();
		////
		//// // rows of block
		//// for (int l = y; l <= y + h; l++) {
